...
//    while(1){
   ...
   //         printf("Enter Image Path: ");
	int i = 1;
	for (i = 1; i < 2294; i = i + 1){
		if (i < 1000){
			if (i < 100){
				if (i < 10){
					sprintf(input,  "/root/darknet/images/output-00000%d.png", i);					
				}
				else {
					sprintf(input,  "/root/darknet/images/output-0000%d.png", i);
				}
			}
			else {
			        sprintf(input, "/root/darknet/images/output-000%d.png", i);
			}
		}
		else
			sprintf(input, "/root/darknet/images/output-00%d.png", i);
		//fflush(stdout);
        	...
        	time=what_time_is_it_now();
	        network_predict(net, X);
        	printf("%d %f", i, what_time_is_it_now()-time);
